...
pub struct QRContainer<A: Scalar> {
...
pub trait QRContainerTraits {
    ...
    fn compress_qr_rank(&self, mut max_rank: usize) -> Result<QRContainer<Self::A>> {
...
macro_rules! qr_container_impl {
    ($scalar:ty) => {
        impl QRContainerTraits for QRContainer<$scalar> {
            type A = $scalar;
            fn get_q(&self) -> ArrayView2<Self::A> {
                self.q.view()
            }
...
qr_container_impl!(f32);
qr_container_impl!(f64);
qr_container_impl!(c32);
qr_container_impl!(c64);
